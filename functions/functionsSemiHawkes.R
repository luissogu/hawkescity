...
# Kernel base function (eucledian space)
...
hist.weighted <- function(x, weight, breaks)
{
  ...
  tempfun <- stepfun(breaks, 0:nb) # f(break_i) = 0:n_b[i] and f(x) = c for x \in (break_i , break_[i+1]) 
  mids <- (breaks[2:nb]+breaks[1:(nb-1)])*.5
  x <- c(x, mids)
  weight <- c(weight, rep(0, nb-1))
  z <- tempfun(x)
  #     print(cbind(weight,x))
